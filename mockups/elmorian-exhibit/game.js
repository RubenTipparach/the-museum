...
(function () {
  ...
  canvasEl.addEventListener('pointermove', function (e) {
    ...
    if (rig.mode === 'room') {
      // In a room the ROOM follows the finger: drag right and the wall slides
      // right with your thumb, so the view turns left. Opposite sign to the
      // inspect branch, which nudges the view rather than the object.
  ...
  function tap(x, y) {
    ...
    switch (u.kind) {
      ...
      case 'plaque': offerCard(L.plaques[u.id].title, L.plaques[u.id].text); inspect(centreOf(h.object), u.normal, fitDist(1.5, 1.1, 4), 'plaque'); audio.click(); break;
