...
try {
  ...
  await tap(size.width * 0.15, size.height - 30);   // the first touch wakes the page, on the floor at the player's feet
  ok((await state()).room === 0 && !(await state()).card, 'a tap on the floor does nothing');
