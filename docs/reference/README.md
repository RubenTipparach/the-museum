...
## The itch track, rendered here

Frames drawn by the real renderer in this sandbox (CLAUDE.md 10), regenerated by
the suites in `docs/ARCHITECTURE.md` section 13; none of them is a mockup.

| Frame | What drew it |
|---|---|
| `elmorian_props.png` | Blender's Workbench, the fourteen props `tools/build_props.py` builds, laid out in a row |
| `itch_room<n>_<name>.png`, `itch_station_<name>.png` | Godot 4.7 GL Compatibility under xvfb at 390x844, `itch/tests/scene_test.gd`: every room and every puzzle wall, the camera where the game puts it |
| `itch_web_boot.png` | the exported Web build in headless Chromium, `itch/tests/web_boot.mjs`: the first frame the browser draws |
| `itch_web_<step>_<portrait|landscape|touch>.png` | the Web build played to its end by `itch/tests/playthrough.mjs`: each puzzle as it was solved, the stones, the arranged house, the ancestor |
