...
| The same script on a GitHub runner | the same frame, 712 ms, so the `render` job in `.github/workflows/build.yml` runs it on every pull request |
...
The itch track (ADR-14). `.github/workflows/build.yml` is the ONE workflow, with the
renderer proof and this as its two jobs, and it deploys on `main`. It is one file
because two files with the same triggers gave every commit two runs:
